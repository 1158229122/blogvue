        ...
        methods: { //事件处理器
            ...
            gotoHome:function(){//用户登录
                ...
                if(!that.emailErr&&!that.passwordErr){
                  this.$http(this.$apiUser.login,{email:that.email,password:that.password}).then(res=>{
                    console.log(res);
                    ...
                    if(res.code==122){//账号未激活
                        that.loginErr = true;
                        that.loginTitle = '该邮箱注册码未激活，请前往邮箱激活';
                      }else if(res.code==123){//账号被封禁
                        that.loginErr = true;
                        that.loginTitle = "该账号已被封禁";
                      }else if(res.code==124){//邮箱注册码未激活已超时
                        // that.loginErr = true;
                        // that.err2005 = true;
                        // that.loginTitle = '该邮箱激活地址已超时，已发送新链接，请前往邮箱激活';
                        that.loginErr = true;
                        that.loginTitle = "用户名或密码错误";
                      }else{
                        that.loginErr = true;
